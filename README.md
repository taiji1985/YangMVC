...
```
假设你的本地调试数据库和生产环境不同，可以在这里判断并给出不同的数据库配置
另外，如果你想启动一个后台Thread，也可以在这里做。
...
在设置好数据后调用renderFreeMarker即可使用Freemarker进行渲染。
如
```java
	public void index(){
		put("name","tomcat");
...
```
其中aa.html是相对于 web.xml中配置的template路径的.回顾一下web.xml中的写法
```xml
  <filter>
    <filter-name>yangmvc</filter-name>
    <filter-class>org.docshare.mvc.MVCFilter</filter-class>
